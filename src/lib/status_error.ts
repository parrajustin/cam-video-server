...
export class StatusError {
    ...
    constructor(
        ...
        this.stackLines = (error ?? new Error()).stack!.split("\n").slice(2);
        if (this.stackLines.length > 0 && this.stackLines[0]!.includes("StatusError")) {
            this.stackLines.shift();
        }

        this._stack = this.stackLines.join("\n");
    }
